...
public class WebSocketRequest extends ExRequest {
	...
	public void accept() throws ExConnectionClosed {
		...
		setHeader("Sec-WebSocket-Accept", handshake.trim());
		if (mProtocol != null) setHeader("Sec-WebSocket-Protocol", mProtocol);
		sendHeaders();
		mAccepted = true;
	}
	...
	public ExWebsocketPacket readFrame() throws ExInvalidWebsocketException, IOException {
		...
		/*
		 * boolean rsv1 = (c[0] & 0x40) == 0x40; boolean rsv2 = (c[0] & 0x20) ==
		 * 0x20; boolean rsv3 = (c[0] & 0x10) == 0x10;
		 */

		byte opcode = (byte) (c[0] & 0xF);

		if (opcode == 0) {
			// Si l'opcode == 0 c'est un packet de continuation, il doit y avoir
			// un packet fragmenté en attente
		...
		if (!fin) {
			...
			// Si l'opcode est différent de 0 alors il faut vérifier qu'aucun
			// packet fragmenté n'est en attente
		...
		else if (len == 127) {// On lit les 4 octets suivants en tant que
								// longueur
			c = new byte[4];
			mClient.read(c);
		...
		if (len > 0) {
			ByteArrayOutputStream payload = new ByteArrayOutputStream(len);
			c = new byte[1];
			int read = 0;
		...
		if (!fin) {
			...
			mFragmentMessage.add(buffer_datas, len);
			ExWebsocketPacket p = new ExWebsocketPacket(mFragmentMessage.getOpcode(), mFragmentMessage.getLength(),
					mFragmentMessage.getBytes());
